...
const UpdateHall = ({ hall, onCancel, onUpdate }) => {
  ...
  useEffect(() => {
    if (hall) {
      ...
      let layout = null;
      try {
        layout = hall.layout ? JSON.parse(hall.layout) : null;
      } catch (error) {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      message.loading('Обновление зала...', 0);
      const response = await axios.put(`/api/Halls/${hall.id}`, hallData, {
      ...
      message.destroy();
      console.error('Ошибка обновления зала:', error.response?.data || error.message);
      message.error(error.response?.data?.message || 'Ошибка обновления зала');
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="row-inputs">
          <h3 className="section-title">Редактирование схемы зала</h3>
